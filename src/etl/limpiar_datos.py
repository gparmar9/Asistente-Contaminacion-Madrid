...
CONTAMINANTES_OBJETIVO = {'NO', 'NO2', 'NOx', 'O3', 'PM10', 'PM2.5'}
...
def limpiar_datos_csv(ruta_input, ruta_estaciones):
    ...
    df_final['validacion'] = df_valid['es_valido']
    ...
    # De momento lo comento para no perder datos, pero habría que ver si nos interesa quedarnos 
    # solo con los datos válidos o no, porque igual nos interesa entrenar el modelo con datos erróneos para que aprenda a identificarlos
    #df_final = df_final[df_final['es_valido'] == 'V'].copy()

    # Mapear nombres de magnitudes
    df_final['magnitud'] = pd.to_numeric(df_final['magnitud'], errors='coerce')
    df_final['contaminante'] = df_final['magnitud'].map(DICCIONARIO_MAGNITUDES)
    df_final = df_final[df_final['contaminante'].isin(CONTAMINANTES_OBJETIVO)]
    ...
    columnas_finales = ['provincia', 'municipio', 'estacion', 'magnitud', 'contaminante','punto_muestreo', 'nombre_estacion', 'ano', 'mes', 'dia', 'hora', 'valor', 'validacion','fecha', 'dia_semana', 'es_fin_semana']
    df_final = df_final[columnas_finales].sort_values(['nombre_estacion', 'fecha'])
...
def limpiar_datos_live(data_live, ruta_estaciones):
    ...
    df_final['estacion'] = pd.to_numeric(df_final['estacion'], errors='coerce')
    df_final = df_final.merge(df_est_temp, left_on='estacion', right_on='CODIGO_CORTO')
